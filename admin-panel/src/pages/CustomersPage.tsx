...
import { Users, Search, Star, Shield, Lock, Unlock, Edit, Wallet } from 'lucide-react';
...
export default function CustomersPage() {
    ...
    const [editForm, setEditForm] = useState({ kyc_level: '', credit_score: 0, max_credit_limit: 0, adjust_credits: 0, is_blocked: false, blocked_reason: '' });

    const fetchCustomers = () => {
        setLoading(true);
        apiClient.get('/admin/customers', { params: { page, page_size: 20, search: search || undefined } })
            .then(res => { setCustomers(res.data.items); setTotal(res.data.total); })
                    ...
                    <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-slate-800 p-6 rounded-xl border border-slate-700 z-50 w-[450px] max-h-[80vh] overflow-y-auto">
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <select value={editForm.kyc_level} onChange={e => setEditForm({ ...editForm, kyc_level: e.target.value })}
                            ...
                            <div>
                                ...
                                <input type="number" value={editForm.credit_score} min={0} max={100}
                            ...
                            <div>
                                ...
                                <input type="number" value={editForm.max_credit_limit}
                            ...
                            <div>
                                ...
                                <input type="number" value={editForm.adjust_credits} step="0.5"
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <button onClick={() => setEditForm({ ...editForm, is_blocked: !editForm.is_blocked })}
                            ...
                            {editForm.is_blocked && (
                                <input placeholder="Motivo do bloqueio" value={editForm.blocked_reason}
                                       onChange={e => setEditForm({ ...editForm, blocked_reason: e.target.value })}
                                       className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm" />
                            )}
                            <button onClick={handleSave} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg font-medium">
                                Guardar Alterações
                            </button>
                        </div>
                    </div>
